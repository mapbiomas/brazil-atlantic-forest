...
var visParMedian3 = {bands: ['ndwi_median', 'ndwi_min', 'gv_median'],gain: [0.08, 0.06, 0.2],gamma: 0.85};
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var herb     = complementares.filter(ee.Filter.eq('reference', 50)).sort('random')//.limit(300)
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(200)
  var naoVeg   = complementares.filter(ee.Filter.eq('reference', 22)).sort('random')
  var aflora   = complementares.filter(ee.Filter.eq('reference', 29)).sort('random').limit(800)
...
var amostraTotal = herb_compl.merge(varzea_compl).merge(camp_compl);
  ...
  var training_camp= mosaicoTotal.select(bandNames).addBands(amostraTotalimg)
                                 .sample({'numPixels': 300, 'region': camp_compl.filterBounds(limite), 'scale': 30, 'seed': 1});
  // Merge training data.
  var training = BDflo.merge(BDreflo).merge(BDcampo)
                      ...
                      .merge(BDagua)
                      // complementary samples
                      .merge(flo).merge(varzea).merge(herb).merge(agro).merge(aflora)
                      // .merge(training_herb)
                      // .merge (training_varzea)
                      // .merge(training_camp);
