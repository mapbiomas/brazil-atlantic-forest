...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var flo      = complementares.filter(ee.Filter.eq('reference',  3)).sort('random').limit(2500)  // flo   
  var sav      = complementares.filter(ee.Filter.eq('reference',  4)).sort('random').limit(1000)  // sav   
  var reflo    = complementares.filter(ee.Filter.eq('reference',  9)).sort('random')              // reflo 
  var varzea   = complementares.filter(ee.Filter.eq('reference', 11)).sort('random').limit(300)   // varzea
  var campo    = complementares.filter(ee.Filter.eq('reference', 12)).sort('random').limit(1500)  // campo 
  var herb     = complementares.filter(ee.Filter.eq('reference', 50)).sort('random')              // herb  
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(2000)  // agro  
  var naoVeg   = complementares.filter(ee.Filter.eq('reference', 22)).sort('random')              // naoVeg
  var aflora   = complementares.filter(ee.Filter.eq('reference', 29)).sort('random').limit(500)   // aflora
  var agua     = complementares.filter(ee.Filter.eq('reference', 33)).sort('random').limit(100)   // agua   
...
//print('COMP. agua  ', agua.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      // complementary samples
                      .merge(flo).merge(sav)
                      .merge(varzea).merge(campo)
                      .merge(agro).merge(aflora).merge(agua);
