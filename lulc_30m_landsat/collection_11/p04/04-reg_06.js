...
'savi_median_dry',	'gcvi_stdDev',	'nir_median_dry',	'savi_median_wet',
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var flo      = complementares.filter(ee.Filter.eq('reference',  3)).sort('random').limit(200)   // flo   
  ...
  var campo    = complementares.filter(ee.Filter.eq('reference', 12)).sort('random').limit(1300)  // campo 
  var herb     = complementares.filter(ee.Filter.eq('reference', 50)).sort('random')              // herb  
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(600)   // agro  
