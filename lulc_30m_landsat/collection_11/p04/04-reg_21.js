...
'green_median','gcvi_median','gcvi_median_wet',	'gcvi_max',	'evi2_median_wet',	'ndwi_max',	'gcvi_median_dry',	
'ndwi_min',	'ndvi_median_wet',	'evi2_min',	'blue_max',	'hallcover_median',	'cai_median_wet',	'ndvi_max',	'evi2_median',	
'swir1_median',	'ndvi_median',	'cai_median',	'ndwi_median',	'green_max',	'ndwi_median_dry',	'red_median',	'gcvi_stdDev',	
'gcvi_min',	'hallheigth_max',	'green_min',	'swir1_min',	'green_median_wet',	'blue_median_wet',	'swir2_median_wet',	'swir2_max',	
'hallheigth_median_dry',	'swir1_max',	'blue_min',	'evi2_amp',	'green_median_texture',	'nir_min',	'ndvi_min',	'savi_amp',
'hallheigth_min',	'cai_min',	'green_median_dry',	'swir1_median_wet',	'blue_median_dry',	'swir2_median_dry',	'blue_median',	
'nir_median_wet',	'red_median_wet',	'savi_max',	'savi_median_wet',	'hallcover_min',	'swir1_stdDev',	'gcvi_amp',	'shade_median_wet',	
'savi_min',	'nir_max',	'savi_median',
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var reflo    = complementares.filter(ee.Filter.eq('reference', 9)).sort('random').limit(600)
  var varzea   = complementares.filter(ee.Filter.eq('reference', 11)).sort('random').limit(1000)
  var campo    = complementares.filter(ee.Filter.eq('reference', 12)).sort('random')
  var herb     = complementares.filter(ee.Filter.eq('reference', 50)).sort('random').limit(650)
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(300)
...
//print('COMP. agua  ', agua.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)
                      ...
                      .merge(BDagua)
                      // complementary samples
                      .merge(flo).merge(reflo).merge(varzea).merge(herb).merge(agro);
