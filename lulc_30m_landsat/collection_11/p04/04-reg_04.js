...
var flo_compl = /* color: #d63000 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
                [[[-48.650887458837346, -28.137105794945214],
                  [-48.65009352496894, -28.13547850437066],
                  [-48.650908916509465, -28.13504329456305]]]),
            {
              "reference": 3,
              "system:index": "0"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-48.681101451089624, -28.160403144434824],
                  [-48.68050063627029, -28.160062624868484],
                  [-48.68178809659744, -28.159495089850548]]]),
            {
              "reference": 3,
              "system:index": "1"
            })]),
    ...
    ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
                [[[-48.662041629638665, -28.174064079792704],
                  [-48.65996023544311, -28.169921547016305],
                  [-48.66116186508178, -28.169448644946275],
                  [-48.66242786773681, -28.17186988149861]]]),
            {
              "reference": 50,
              "system:index": "0"
            })]);
...
'blue_median',	'blue_median_wet',	'ndvi_stdDev',	'hallheigth_median',	'blue_max',	'swir2_median',
...
var batch1 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b1_reg_04')
var batch2 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b2_reg_04')
var batch3 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b3_reg_04')
var batch4 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b4_reg_04')
var batch5 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b5_reg_04')
var batch6 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b6_reg_04')
var batch7 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b7_reg_04')
var batch8 = ee.FeatureCollection('projects/ee-yasmin-mapbiomas/assets/LANDSAT_MataAtlantica/COL11/amostras/amostras_complementares_b8_reg_04')
...
// print(complementares.filter(ee.Filter.eq('reference',11)).size())
  ...
  var reflo    = complementares.filter(ee.Filter.eq('reference', 9)) .sort('random').limit(50)
  var varzea   = complementares.filter(ee.Filter.eq('reference', 11)).sort('random').limit(500)
  var campo    = complementares.filter(ee.Filter.eq('reference', 12)).sort('random').limit(300)
  var herb     = complementares.filter(ee.Filter.eq('reference', 50)).sort('random').limit(200)
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(100)
  var naoVeg   = complementares.filter(ee.Filter.eq('reference', 22)).sort('random')
  var aflora   = complementares.filter(ee.Filter.eq('reference', 29)).sort('random').limit(230)
...
var amostraTotal = flo_compl.merge(herb_compl)
  ...
  var training_flo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg)
                                 .sample({'numPixels': 200, 'region': flo_compl.filterBounds(limite), 'scale': 30, 'seed': 1});
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      // complementary exportadas
                      .merge(reflo).merge(varzea)
                      .merge(campo).merge(herb).merge(agro).merge(aflora)
                      .merge(training_flo).merge(training_herb);
...
if (coleta) {  Map.addLayer(classified, vis2, 'RF'+ano+"_"+regiaoID, false);}
...
//Map.setCenter(-48.85977, -26.48297, 11)
