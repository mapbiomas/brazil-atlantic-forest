...
'shade_median',	'swir2_min',	'ndvi_median_dry',
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var reflo    = complementares.filter(ee.Filter.eq('reference', 9)).sort('random').limit(800)
  ...
  var agro          = complementares.filter(ee.Filter.eq('class_name', 'agro')).sort('random')
  var agro_2000     = complementares.filter(ee.Filter.eq('class_name', 'agro_2000')).sort('random')
  var agro_2010     = complementares.filter(ee.Filter.eq('class_name', 'agro_2010')).sort('random')
...
//print('COMP. agua  ', agua.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)
                      ...
                      .merge(BDagua)
                      // complementary samples
                      .merge(flo).merge(reflo).merge(campo).merge(agro)
...
//Map.setCenter(-50.894064, -28.316352, 10)
