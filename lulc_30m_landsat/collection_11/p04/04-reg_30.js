...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var flo      = complementares.filter(ee.Filter.eq('reference', 3)).sort('random').limit(400)
  var sav      = complementares.filter(ee.Filter.eq('reference', 4)).sort('random')
  var reflo    = complementares.filter(ee.Filter.eq('reference', 9)).sort('random').limit(700)
  var varzea   = complementares.filter(ee.Filter.eq('reference', 11)).sort('random').limit(900)
  var campo    = complementares.filter(ee.Filter.eq('reference', 12)).sort('random').limit(600)
...
//print('COMP. agua  ', agua.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)
                      ...
                      .merge(BDagua)
                      // complementary samples
                      .merge(flo).merge(reflo).merge(varzea).merge(campo).merge(agro).merge(aflora).merge(agua)
