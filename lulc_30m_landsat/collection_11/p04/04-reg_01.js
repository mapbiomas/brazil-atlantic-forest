...
'nir_median',	'gcvi_median_dry',	'red_median',	'ndwi_max',	'gcvi_amp',	'nir_median_wet',	'gcvi_max',	'cai_median',
'blue_median',	'cai_median_wet',	'evi2_max',	'cai_max',	'ndwi_min',	'gcvi_median_wet',	'swir1_median_dry',	
'gcvi_min',	'swir1_median',	'red_median_wet',	'ndvi_median_dry',	'ndwi_stdDev',	'savi_median',	'red_max',	
'hallcover_stdDev',	'red_amp',	'cai_min',	'gv_stdDev',	'savi_stdDev',	'hallcover_min',	'swir1_max',	
'evi2_median_wet',	'swir2_median',	'swir2_median_wet',	'hallheigth_median_dry',	'nir_max',	'ndwi_median',
'red_median_dry',	'wefi_median_wet',	'gvs_median_wet',	'savi_median_dry',	'evi2_median',	'gcvi_median',
'nir_median_dry',	'cai_median_dry',	'sefi_stdDev',	'cai_amp',	'hallcover_median',	'hallheigth_max',	
'green_min',	'green_median',	'blue_median_dry',	'swir2_max',	'fns_median_dry',	'blue_max',	
'savi_median_wet',	'shade_stdDev',	'ndvi_max',	'hallcover_max',	'swir2_min',
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var flo      = complementares.filter(ee.Filter.eq('reference', 3)).sort('random').limit(750)
  var sav      = complementares.filter(ee.Filter.eq('reference', 4)).sort('random').limit(600)
  ...
  var campo    = complementares.filter(ee.Filter.eq('reference', 12)).sort('random').limit(1200)
...
//print('COMP. agua  ', agua.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)
                      .merge(BDsav).merge(BDherb)
                      .merge(BDaflora).merge(BDagro)
                      .merge(BDnaoVeg).merge(BDagua)
                      .merge(BDvarzea)
                      // complementary samples
                      .merge(campo).merge(agro)
                      .merge(sav).merge(flo);
