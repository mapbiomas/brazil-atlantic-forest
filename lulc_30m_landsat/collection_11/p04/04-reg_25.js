...
var varzea_compl = /* color: #d63000 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
                [[[-51.72960880853452, -21.158552230508494],
                  [-51.73235539056577, -21.149907180108396],
                  [-51.751238142030616, -21.16143380179888]]]),
            {
              "reference": 11,
              "system:index": "0"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-51.683603559511084, -21.13101438799566],
                  [-51.68566349603452, -21.1438233235922],
                  [-51.68188694574155, -21.14126162504507]]]),
            {
              "reference": 11,
              "system:index": "1"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-51.93766239740171, -21.282409062686362],
                  [-51.93663242913999, -21.2648130115869],
                  [-51.9479620800189, -21.293285568923498]]]),
            {
              "reference": 11,
              "system:index": "2"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-52.2177560998615, -21.84534552344642],
                  [-52.23114568726384, -21.84215879487289],
                  [-52.233548946541184, -21.86542027925835]]]),
            {
              "reference": 11,
              "system:index": "3"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-52.39868719117009, -21.978168746976433],
                  [-52.406583614509934, -21.969890825995158],
                  [-52.411046810310715, -21.99153975356308]]]),
            {
              "reference": 11,
              "system:index": "4"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-52.45808202759587, -22.128681898151388],
                  [-52.46391851441228, -22.12009472766267],
                  [-52.4652918054279, -22.131862198871694]]]),
            {
              "reference": 11,
              "system:index": "5"
            })]);
...
'green_max',	'swir2_median_dry',	'hallcover_min',	'nir_max',	'swir1_median_wet',	'savi_median_wet',	'green_median',	'gcvi_max',	
'green_median_wet',	'gcvi_median_wet',	'red_median_dry',	'savi_median',	'ndvi_median',	'ndwi_stdDev',	'swir2_median_wet',	'cai_median',	
'swir1_median_dry',	'hallcover_median_dry',	'savi_max',	'hallheigth_median',		'ndvi_median_wet',	'gv_stdDev',	'cai_median_wet',	
'evi2_median',	'evi2_max',	'swir1_min',	'cai_min',	'ndfi_stdDev',	'nir_stdDev',	'shade_min',	'ndwi_max',	'swir2_stdDev',	'nir_median',	
'swir2_median',	'gcvi_amp',	'evi2_median_wet',	'red_min',	'red_median_wet',	'savi_amp',	'hallheigth_median_wet',	'gcvi_stdDev',	'latitude',	
'red_max',	'gcvi_median',	'fns_min',	'hallcover_max',	'blue_median',	'ndwi_median',	'hallcover_median_wet',	'hallheigth_median_dry',	
'savi_stdDev',	'gcvi_min',	'ndwi_amp',	'nir_min',	'swir1_median',	'evi2_stdDev',	'red_median',
...
//print('BDagua  ', BDagua.size())
  ...
  var complementares = ee.FeatureCollection(dirsamples + 'amostras_comp_2020')
  // var complementares = ee.FeatureCollection(dirsamples + 'amostras_comp_2020_v'+v_comp_in)
                     .filter(ee.Filter.eq('reg_id', regiaoID));
                           
  var flo      = complementares.filter(ee.Filter.eq('reference', 3)).sort('random').limit(900)
  ...
  var varzea   = complementares.filter(ee.Filter.eq('reference', 11)).sort('random').limit(2000)
...
var amostraTotal = varzea_compl
  ...
  var training_varzea = mosaicoTotal.select(bandNames).addBands(amostraTotalimg)
                                 .sample({'numPixels': 300, 'region': varzea_compl.filterBounds(limite), 'scale': 30, 'seed': 1});
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)
                      ...
                      .merge(BDagua)
                      // complementary samples
                      .merge(flo).merge(varzea).merge(agro).merge(training_varzea)
