...
var compl_campo = /* color: #98ff00 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
                [[[-52.36701624685958, -28.82957546556454],
                  [-52.37594263846114, -28.821980730728882],
                  [-52.38392489248946, -28.84115448977776]]]),
            {
              "reference": 12,
              "system:index": "0"
            })]);
...
var colBruta = ee.Image('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-10/GENERAL/classification-mat-ft/MA_col10_p06a_v5')
                .select('classification_'+anos).remap(remap_in,remap_out).rename('classification_'+anos).clip(limite);
...
'swir1_stdDev',	'gcvi_min',	'savi_max',	'nir_amp',	'ndwi_median_wet',	'nir_median_wet',	'cai_max',	
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(400)   // agro  
...
var amostraTotal = compl_campo//.merge(agro);//.merge(flo).merge(campo)//.merge(aflora).merge(nao_flo)
  
  // Convert the merged feature collection into an image where each pixel's value represents the 'reference' property of the nearest feature.
  // This creates a reference image for sampling.
  ...
  // Sample the mosaic image using points from the complementary samples.
  // The number of pixels to sample is specified.
  // The sampled data will include the band values from `mosaicoTotal` and the 'reference' band from `amostraTotalimg`.
  var training_campo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 300, 'region': compl_campo.filterBounds(limite), 'scale': 30, 'seed': 1});
