...
var agro_compl = /* color: #d52ef3 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
                [[[-48.49682949952742, -21.60274926790533],
                  [-48.49631451539656, -21.606100906352033],
                  [-48.487388123795, -21.60179164265994]]]),
            {
              "reference": 21,
              "system:index": "0"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-48.49442624025008, -21.646793530979654],
                  [-48.492022980972735, -21.63929424892775],
                  [-48.4957995312657, -21.64073031181012]]]),
            {
              "reference": 21,
              "system:index": "1"
            }),
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-48.505342095627064, -21.652856495514666],
                  [-48.502595513595814, -21.65796195231748],
                  [-48.49778899504113, -21.65094190264765]]]),
            {
              "reference": 21,
              "system:index": "2"
            })]);
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var flo      = complementares.filter(ee.Filter.eq('reference', 3)).sort('random').limit(150)
  ...
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(2000)
...
var amostraTotal = agro_compl
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)
                      ...
                      .merge(BDagua)
                      // complementary samples
                      .merge(flo).merge(varzea).merge(agro) .merge(training_agro);
