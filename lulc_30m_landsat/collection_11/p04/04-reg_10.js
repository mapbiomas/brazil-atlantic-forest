...
'blue_max',	'ndfi_median',	'swir2_max',
...
var batch8 = ee.FeatureCollection('projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/amostras_complementares_b8_'+regiaoID)
  ...
  var reflo    = complementares.filter(ee.Filter.eq('reference',  9)).sort('random').limit(1000)  // reflo
  var varzea   = complementares.filter(ee.Filter.eq('reference', 11)).sort('random').limit(700)   // varzea
  var campo    = complementares.filter(ee.Filter.eq('reference', 12)).sort('random').limit(900)   // campo 
  var herb     = complementares.filter(ee.Filter.eq('reference', 50)).sort('random')              // herb  
  var agro     = complementares.filter(ee.Filter.eq('reference', 21)).sort('random').limit(1500)  // agro  
  ...
  var agua     = complementares.filter(ee.Filter.eq('reference', 33)).sort('random').limit(200)   // agua   
...
print('COMP. agua  ', agua.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      // complementary samples
                      .merge(flo).merge(reflo)
                      .merge(varzea).merge(campo)
                      .merge(agro).merge(agua);
