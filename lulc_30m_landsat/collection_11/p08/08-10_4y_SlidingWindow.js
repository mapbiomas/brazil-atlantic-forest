 ...
 * OBJECTIVE: Apply a 4-year sliding window temporal filter to the classification results.
 * 
 * DESCRIPTION:
 * This script performs a 4-year temporal filtering aiming to remove noise of wrongly assigned classes
 * up to two consecutive years. Lists of years are created to handle odd and even years separately for the temporal filtering.
...
var descricao = '4 years sliding window';
...
var anosP = ee.List.sequence(2024,1988,-2)                                                           
                  .map(function(y){                                                     
                        return ee.Number(y).int(); });
print(anosP);
print(anosI);
...
var window4y = function(img, classe){
                  ...
                  var class_final = img.select(ee.String('classification_').cat(ultimo)).rename(ee.String('00_classification_').cat(ultimo));
                      class_final = class_final.addBands(
                                    img.select(ee.String('classification_').cat(penult)).rename(ee.String('00_classification_').cat(penult)));
                          print(class_final, 'class1');                     
