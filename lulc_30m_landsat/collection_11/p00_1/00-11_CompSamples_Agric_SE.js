 ...
 * OBJECTIVE: Extraction of complementary training samples for land use and land cover classification, focusing on agricultural areas in the Southeast region.
 * 
 * DESCRIPTION:
 * This script automates the collection of multi-temporal training samples across the Atlantic Forest biome 
 * specifically tailored for the Southeast agricultural domain ('agric_SE'). It loops over historical Landsat 
...
var agro_1985 = /* color: #d63000 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
        ...
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-43.14529240987417, -21.055110158146153],
                  ...
                  [-43.14718068502066, -21.049663242976344],
    ...
    agro_2000 = /* color: #98ff00 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
    ...
    agro_compl = /* color: #00fff1 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
...
// Set the region identifier for the Southeast agricultural sampling processing
var regiaoID = 'agric_SE';

// Manually defined base year for standalone executions or testing
var ano = 2020; 

// Output Asset directory for storing generated complementary training sample feature collections
var dirout = 'projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/GENERAL/SAMPLES/MATAATLANTICA/';

// Import the official auxiliary feature collection containing regional boundaries for the 2025 classification
var regioesCollection = ee.FeatureCollection('projects/mapbiomas-workspace/AUXILIAR/Mata_Atlantica_Regioes2025');

// Assign the study area boundary geometry corresponding to the Southeast agricultural region
var limite = agric_SE;

// Import previously exported complementary samples asset for validation or feature comparison
var amostras_exportadas = ee.FeatureCollection(dirout + 'amostras_complementares_'+regiaoID);

// Bounding polygon covering the geographical extent of the Atlantic Forest biome for mosaic clipping
...
// Visualization configuration for false-color RGB display (SWIR1, NIR, Red)
var visParMedian2 = {
...
// Batch configuration array grouping multi-year ranges for sequential execution
var dicExp = [
...
// Load the external script responsible for generating cloud-free monthly Landsat mosaics
...
// Import multi-temporal cluster assets containing textural and NDFI statistical features
...
// Load the collection of standardized MapBiomas grid mosaics covering Brazil
var exportMos = ee.ImageCollection('projects/mapbiomas-mosaics/assets/LANDSAT/LULC/BRAZIL/MA-mosaics-32days-grids');

// Loop through each batch definition to process assigned year groups
for (var i = 0; i < dicExp.length; i++) {
  ...
  // Loop through each individual year within the active batch
  for (var i_ano=0; i_ano<anos.length; i_ano++){
    var ano = anos[i_ano];
    
    // Define standard EPSG:4326 projection at 30-meter resolution
    var projLandsat = ee.Projection('EPSG:4326').atScale(30);
    
    // Fetch and reproject the cloud-free Landsat mosaic for the target year
    var mosaicoTotal = cloudMos.getMosaic(ano, limite_MA)
                               .reproject({crs: projLandsat}); 

    // Combine standard mosaic bands, textural clusters, NDFI amplitude, and median clusters
    ...
    // List of spectral indices, statistical metrics, fractions, canopy variables, and terrain features for classification
    var bandNames = ee.List([
      ...
      'hallcover_max', 'swir1_amp', 'latitude', 'longitude', 'slope',
    ]);
    ...
    // Dynamically increase agricultural sample sizes according to historical land expansion thresholds
    var num_agric = 3500;
    ...
    var classConfigs = [
      ...
      {variable: typeof agro         !== 'undefined' ? agro         : null, number:         0, ref: 19, name: 'agro'},
      {variable: typeof agro_compl   !== 'undefined' ? agro_compl   : null, number:       600, ref: 19, name: 'agro_compl'},
      {variable: typeof agro_1985    !== 'undefined' ? agro_1985    : null, number: num_agric, ref: 19, name: 'agro_1985'},
      {variable: typeof agro_2000    !== 'undefined' ? agro_2000    : null, number: num_agric, ref: 19, name: 'agro_2000'},
    ...
    // Filter the configuration array to keep only defined variables with sample allocations greater than zero
    var classesAtivas = classConfigs.filter(function(item) {
      return item.variable !== null && item.number > 0;
    });

    // Format active class collections by assigning target reference values and the current processing year
    ...
    // Merge all individual class collections into a single flattened FeatureCollection
    var amostraTotal = ee.FeatureCollection(listaCollections).flatten();

    // Convert vector reference collection into a single-band raster image encoding class reference IDs
    ...
    // --- 3. SAMPLING FUNCTION ---
    // Function designed to extract raster pixel values using spatial geometries per active class
    var getTrainingSamples = function(classObj) {
      // Clip feature geometries to the spatial boundary of interest
      var regionClip = classObj.variable.filterBounds(limite);
      
      // Sample multi-band raster data at 30m resolution using dynamic annual random seeds
    ...
    // Collect extracted sample feature collections across all active classes
    var listaAmostras = classesAtivas.map(function(item) {
      return getTrainingSamples(item);
    });

    // Flatten multi-class sample results into a unified annual collection
  ...
  // Export the final merged multi-year batch collection to an Earth Engine Asset
...
// Output the list of processed class names to the Earth Engine console
