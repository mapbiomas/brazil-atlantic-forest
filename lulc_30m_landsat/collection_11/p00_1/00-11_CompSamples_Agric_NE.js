 ...
 * OBJECTIVE: Extraction of complementary training samples for land use and land cover classification, focusing on agricultural areas in the Northeast region.
 * 
 * DESCRIPTION:
 * This script automates the collection of multi-temporal training samples across the Atlantic Forest biome 
 * specifically tailored for the Northeast agricultural domain ('agric_NE'). It loops over historical Landsat 
...
var agric_NE = 
    ...
    agro_1985 = /* color: #ffc82d */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
...
// Unique identifier for the specific region or class group being processed
...
// Import of the auxiliary feature collection containing regional boundaries for the Atlantic Forest
var regioesCollection = ee.FeatureCollection('projects/mapbiomas-workspace/AUXILIAR/Mata_Atlantica_Regioes2025');

// Definition of the spatial limit for sampling, set to the specific agricultural region variable
var limite = agric_NE;
...
// Import the modular script responsible for generating cloud-free monthly Landsat mosaics
...
// Load saved cluster images which provide textural and spectral segmentation context for classification
...
// Load the collection of standardized MapBiomas mosaics for the Brazilian territory
var exportMos = ee.ImageCollection('projects/mapbiomas-mosaics/assets/LANDSAT/LULC/BRAZIL/MA-mosaics-32days-grids');

// Iterate through the defined batches of years
for (var i = 0; i < dicExp.length; i++) {
  var config = dicExp[i]; 
  var batch = config.batch;
  var anos = config.anos;

  // Nested loop to process each year within the current batch
  for (var i_ano=0; i_ano<anos.length; i_ano++){
    var ano = anos[i_ano];
    
    // Set the projection to WGS84 with a 30-meter resolution for consistency across Landsat datasets
    var projLandsat = ee.Projection('EPSG:4326').atScale(30);
    // Generate the yearly mosaic for the Atlantic Forest region
    ...
    // List of spectral bands and indices selected for the agricultural classification process
    ...
    // Initialize the number of samples for agriculture and increase based on the temporal threshold
    var num_agric = 1000;
    ...
    // Mapping of classification variables to their respective sample quantities and reference IDs
    var classConfigs = [
      ...
      {variable: typeof agro_2000    !== 'undefined' ? agro_2000    : null, number:         0, ref: 21, name: 'agro_2000'},
    ...
    // Filter the configuration list to keep only the classes that are defined and have samples to be collected
    var classesAtivas = classConfigs.filter(function(item) {
      return item.variable !== null && item.number > 0;
    });

    // Create a list of FeatureCollections where each feature is tagged with its class reference and year
    ...
    // Merge all class-specific FeatureCollections into a single unified collection
    var amostraTotal = ee.FeatureCollection(listaCollections).flatten();

    // Rasterize the feature collection to create a reference image where pixels contain the class ID
    ...
    // --- 3. SAMPLING FUNCTION ---
    // Function that performs stratified sampling over the mosaic using the provided class configuration
    var getTrainingSamples = function(classObj) {
      // Intersect the class polygons with the study area limit
      var regionClip = classObj.variable.filterBounds(limite);
      
      // Select the required bands, attach the reference label, and extract pixels
      var samples = mosaicoTotal.select(bandNames)
        ...
        .sample({
          ...
          'region': regionClip,
          'scale': 30,
          'seed': 1, // Constant seed ensures spatial consistency in sampling
          'geometries': true
        });

      // Attach metadata for identification and add a random column for training/validation partitioning
    ...
    // Execute sampling for each active class in the current year
    var listaAmostras = classesAtivas.map(function(item) {
      return getTrainingSamples(item);
    });

    // Flatten all samples from different classes into a single year-specific collection
    var amostrasComplementaresFinal = ee.FeatureCollection(listaAmostras).flatten();

    // Accumulate the current year's samples into the batch-wide collection
    if (i_ano == 0){  
      // Initialize the collection with the first year of the loop
      var amostras = amostrasComplementaresFinal;
    }
    else { 
      // Merge samples from subsequent years into the existing collection
      amostras = amostras.merge(amostrasComplementaresFinal);
    }
    
  } // End of inner year loop

  // Export the aggregated batch collection to an Earth Engine Asset for later use
...
// Console output displaying the list of names for all classes that were processed
