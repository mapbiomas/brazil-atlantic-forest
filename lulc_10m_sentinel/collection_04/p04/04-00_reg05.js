...
// Define the region ID (reg_05)
var regiaoID = 'reg_05'; // old 25
...
// If collecting data, only process 2019
...
// Load the version 1 classification image for reference
var v1 = ee.Image('projects/mapbiomas-workspace/COLECAO3_S2/MATA_ATLANTICA/regioes/reg_05_v1')
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'cvi_median','afvi_median_dry','evi_median_dry','swir2_median','cvi_median_wet','ndvi_median_wet','spri_median','red_edge_2_median','nddi_median_dry','nddi_median_wet','evi_median','ri_median_wet','gvmi_median_dry','msi_median','mbi_median_wet','mbi_median_dry','blue_median','swir2_stdDev','gemi_median',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(campo).merge(agro).merge(flo).merge(herb).merge(reflo).merge(agua)
  ...
  // Select the reference band from the reduced image
  ...
  // var training_sav = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': sampleComplementar, 'region': savanna_r23.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_cam = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 2500, 'region': campo.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_agr = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 2500, 'region': agro.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the herbaceous sandbank class
  var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 500, 'region': herb.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the reforestation class
  var training_reflo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 500, 'region': reflo.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the water class
  var training_agua = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 800, 'region': agua.filterBounds(limite), 'scale': 10, 'seed': 1});
...
//print(training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo).merge(BDherb)//.merge(BDOutroNFlo).merge(BDAflora)
                      .merge(BDagro).merge(BDNaoVeg).merge(BDagua)
                      //complementares
                      .merge(training_cam).merge(training_var).merge(training_agr).merge(training_flo).merge(training_herb).merge(training_reflo).merge(training_agua)
  ...
  if (coleta) {  
    ...
    // Add the version 1 reference classification layer to the map
    Map.addLayer(v1.select('classification_'+ano), vis, 'v1', true);
  }
