...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'mbi_median_dry','red_edge_2_median','red_median_wet','ui_median','gemi_median','spri_median','blue_median','shape_median','swir2_median','afvi_median','ndvi_median_wet','swir2_median_wet','osavi_median_wet','iia_median','cvi_median','msi_median_wet','dswi5_median_dry','gli_median','spri_median_1',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(herb).merge(reflo).merge(flo).merge(agro).merge(sav) //.merge(agua)
  ...
  // var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 300, 'region': nao_flo.filterBounds(limite), 'scale': 10, 'seed': 1});
...
print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo).merge(BDsav).merge(BDherb)//.merge(BDAflora)
