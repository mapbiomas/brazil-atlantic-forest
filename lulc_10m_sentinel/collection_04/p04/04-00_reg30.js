...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(aflora).merge(agua).merge(flo).merge(agro).merge(campo).merge(reflo).merge(naoveg)
  ...
  var training_afl = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 800, 'region': aflora.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_agu = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 400, 'region': agua.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_naoveg = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 500, 'region': naoveg.filterBounds(limite), 'scale': 10, 'seed': 1});
...
print(training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      //complementares
                      .merge(training_afl).merge(training_var).merge(training_agu).merge(training_flo).merge(training_agr).merge(training_cam).merge(training_reflo).merge(training_naoveg)
  ...
  Map.addLayer(col10.select('classification_'+ano), vis, 'Col 10 Landsat LULC', false)
  }
