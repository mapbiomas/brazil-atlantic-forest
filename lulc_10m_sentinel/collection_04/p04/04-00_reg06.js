...
// Define the region ID (reg_06)
var regiaoID = 'reg_06'; // old 25
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'blue_median','blue_median_wet','ui_median','shape_median','cvi_median','mbi_median_dry','red_median_wet','swir2_median_dry','swir1_median','ndvi_median','red_edge_2_median','ratio_median_wet','wetness_median_dry','swir2_median_wet','afvi_median_dry','red_stdDev','gvmi_median_1','msi_median','mbi_median_wet',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = agro.merge(flo).merge(campo).merge(varzea)//.merge(aflora).merge(nao_flo)
...
//print(training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      //complementares
                      .merge(training_var)
                      .merge(training_cam).merge(training_agr).merge(training_flo)//.merge(training_agu)//.merge(training_afl).merge(training_agr)
