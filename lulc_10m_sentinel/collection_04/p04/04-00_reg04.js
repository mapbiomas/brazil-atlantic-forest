...
var v1 = ee.Image('projects/mapbiomas-workspace/COLECAO3_S2/MATA_ATLANTICA/regioes/reg_04_v1')
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'blue_median','blue_median_wet','spri_median','osavi_median','lai_median','red_edge_1_median','evi_median','swir1_median','evi_median_wet','bsi_median_2','ratio_median','evi_median_dry','red_median','osavi_median_wet','ui_median_wet','ri_median_wet','green_median','red_edge_2_median','cvi_median',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = agro.merge(varzea).merge(campo).merge(aflora).merge(herb).merge(naoVeg).merge(flo).merge(reflo);
  ...
  var training_var = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 900, 'region': varzea.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 700, 'region': herb.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the non-vegetated class
  var training_naoVeg = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 300, 'region': naoVeg.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_reflo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 700, 'region': reflo.filterBounds(limite), 'scale': 10, 'seed': 1});
...
//print(training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDherb)//.merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      .merge(training_agr).merge(training_var).merge(training_cam).merge(training_afl)
                      .merge(training_herb).merge(training_naoVeg).merge(training_flo).merge(training_reflo);
  ...
  if (coleta) {  
    ...
    Map.addLayer(v1.select('classification_'+ano), vis, 'v1'+ano, true);
  }
