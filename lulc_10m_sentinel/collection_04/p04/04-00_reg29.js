...
// Define the years to process (2017-2024)
var anos = [
...
Map.addLayer(mapa_estavel_col9, vis, 'mapa_estavel_col9', false);
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'brightness_median_dry','dswi5_median_wet','gcvi_median','gcvi_median_dry','gli_median_dry','green_median_texture','green_median_wet',
                  'lswi_median','mbi_median','ndwi_median_wet','red_edge_4_median_wet','red_median','ri_median_wet','spri_median','spri_median_1',
                  'spri_median_wet','swir1_median_wet','swir2_median','ui_median','wetness_median_dry',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(agro).merge(flo).merge(savana).merge(campo).merge(agua).merge(reflo)
  ...
  var training_flo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 2000, 'region': flo.filterBounds(limite), 'scale': 10, 'seed': 1});
  // var training_nflo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 300, 'region': restinga.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic using the savanna points
  var training_sav = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1500, 'region': savana.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_cam = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 100, 'region': campo.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training_agr = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 900, 'region': agro.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic using the water points
  var training_agua = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 100, 'region': agua.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic using the forest plantation
  var training_reflo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 100, 'region': reflo.filterBounds(limite), 'scale': 10, 'seed': 1});
...
// Train a Random Forest classifier
  ...
  Map.addLayer(outline_gridbr.clip(limite.geometry()), visPar_gridbr, 'Grid', true);
  }
