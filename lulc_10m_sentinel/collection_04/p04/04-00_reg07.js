...
// If collecting data, only process 2024
if (coleta) {var anos = [2024]}
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'mbi_median_wet','lswi_median','evi_median_dry','gvmi_median_dry_1','gemi_median','awei_median_wet','afvi_median_wet','mbi_median_dry','cvi_median','swir1_median','lswi_median_wet','ndwi_median_dry','brba_median_wet','wetness_median_dry','ri_median_wet','nddi_median_dry','gvmi_median_wet_1','nddi_median','gvmi_median_wet',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(agro).merge(agua).merge(campo)//.merge(aflora).merge(nao_flo)
  ...
  var training_agua = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 400, 'region': agua.filterBounds(limite), 'scale': 10, 'seed': 1});
...
//print(training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      //complementares
                      .merge(training_var).merge(training_agr).merge(training_agua)
