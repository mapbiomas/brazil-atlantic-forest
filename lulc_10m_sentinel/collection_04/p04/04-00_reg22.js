...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'green_median_wet','green_median','red_edge_1_median','swir1_stdDev','red_median','swir2_median_wet','wetness_median','spri_median_1','swir2_median_dry','blue_median_wet','swir1_median','blue_median','gvmi_median_wet','gvmi_median_dry_1','spri_median','brightness_median','bsi_median_2','spri_median_wet','afvi_median_dry',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = savana.merge(campo).merge(flo).merge(agro).merge(afl).merge(varzea) //flo.merge(agr).merge(agua)
  ...
  // Sample the mosaic for the aflora class
   var training_afl = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1000, 'region': afl.filterBounds(limite), 'scale': 10, 'seed': 1});
...
print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo).merge(BDvarzea)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      .merge(training_cam)
                      .merge(training_sav).merge(training_agr).merge(training_flo).merge(training_var).merge (training_afl)
                      // .merge(training_nflo)
