...
// If collecting data, restrict the processing to the year 2023
if (coleta) {var anos = [2023]}

// Define the number of trees for the Random Forest classifier (100)
var RFtrees = 100;

// Set the tree count to 100 if collecting data
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'swir2_median_dry','rvi_median_1','afvi_median','co2flux_median','brba_median','spri_median_1','mbi_median_wet','lai_median','gli_median_dry','red_edge_1_median','spri_median','iia_median_wet','gvmi_median','red_median','green_median','dswi5_median','red_median_dry','red_edge_1_median_dry','ratio_median_dry',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = campo.merge(reflo).merge(flo).merge(agro).merge (varzea)//.merge(agua)
  ...
  var training_agr = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1000, 'region': agro.filterBounds(limite), 'scale': 10, 'seed': 1});
