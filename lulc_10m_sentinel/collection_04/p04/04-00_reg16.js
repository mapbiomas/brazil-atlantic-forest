...
// Define the region ID (reg_16)
var regiaoID = 'reg_16'; // old 11
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'gli_median','msi_median_wet','ndwi_median','awei_median_wet','ndwi_median_wet','afvi_median','swir2_median_wet','mbi_median_dry','brightness_median','ratio_median_dry','nir_median_dry','red_median','ri_median_wet','red_edge_1_median_dry','green_median','msi_median_dry','gvmi_median','awei_median','rvi_median_1',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(aflora).merge(flo).merge(agro).merge(herb).merge(varzea).merge(campo)
  ...
  var training_flo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 700, 'region': flo.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  // Sample the mosaic for the floodplain class
  ...
  var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 400, 'region': herb.filterBounds(limite), 'scale': 10, 'seed': 1});
...
print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo).merge(BDherb)//.merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      .merge(training_herb).merge(training_cam)
