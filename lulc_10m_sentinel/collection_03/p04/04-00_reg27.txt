...
// Define the region ID (reg_27)
var regiaoID = 'reg_27'; // old 11
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(herb).merge(savana).merge(flo).merge(agro) .merge(varzea)//.merge(agua)
  ...
  var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 600, 'region': herb.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the savanna class
  var training_sav = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 2200, 'region': savana.filterBounds(limite), 'scale': 10, 'seed': 1});
  // // Sample the mosaic for the aflora class (optional)
  ...
  var training_var = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1300, 'region': varzea.filterBounds(limite), 'scale': 10, 'seed': 1});
  // // Sample the mosaic for the agua class (optional)
  ...
  // // Sample the mosaic for the campo class (optional)
  ...
  // // Sample the mosaic for the non-forest class (optional)
  // var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 500, 'region': restinga.filterBounds(limite), 'scale': 10, 'seed': 1});
...
print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo).merge(BDherb)//.merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      .merge(training_var)
                      .merge(training_sav)
                      .merge(training_herb).merge(training_agr).merge(training_flo)
                      // // Merge the agua and agro samples (optional)
