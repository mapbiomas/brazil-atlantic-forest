...
// Define the region ID (reg_10)
var regiaoID = 'reg_10'; // old 11
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'red_edge_2_median','mbi_median_wet','swir1_median','brba_median_wet','gcvi_median_dry','osavi_median','shape_median_dry','swir1_median_dry','brightness_median_dry','gemi_median','brightness_median','cvi_median','mbi_median_dry','red_edge_4_median_wet','blue_median_wet','red_edge_3_median_dry','red_edge_2_median_wet','lswi_median','osavi_median_wet',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(flo).merge(agro).merge(campo).merge(aflora).merge(reflo)
  ...
  // If collecting data, print the number of training points for each class
  if (coleta) {
...
print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      .merge(training_cam).merge(training_afl).merge(training_reflo)//.merge(training_agu)
