...
// Define the region ID (reg_08)
var regiaoID = 'reg_08'; // old 25
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'ndvi_median','red_edge_4_stdDev','swir2_median_dry','wetness_median','gvmi_median_1','bsi_median_2','red_median_wet','red_edge_1_median_wet','evi_median_wet','gvmi_median_wet_1','red_edge_1_median_dry','mbi_median_wet','green_median','swir2_median_wet','cvi_median_dry','gvmi_median_dry','cvi_median','mbi_median_dry','ndwi_median_wet',
...
// Add the clipped mosaic layer to the map for visual check
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = campo.merge(reflo).merge(agro).merge(varzea).merge(flo)//.merge(campo)//.merge(aflora).merge(nao_flo)
  
  // Convert the training points to an image
...
// print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      //complementares
                       .merge(training_var)
                      .merge(training_cam).merge(training_reflo).merge(training_agr).merge(training_flo)//.merge(training_agu)//.merge(training_afl).merge(training_agr)
