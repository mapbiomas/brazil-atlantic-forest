...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'awei_median','blue_median_wet','brightness_median_dry','gcvi_median_wet','iia_median','mbi_median','nddi_median','nddi_median_wet',
                  'ndwi_median_wet','red_edge_2_median_dry','red_edge_3_median','red_edge_4_median','red_edge_4_median_dry','red_median_wet',
                  'rvi_median_wet','spri_median_1','swir1_median_wet','swir2_median_wet','ui_median_wet','wetness_median',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = campo.merge(aflora).merge(flo).merge(agro).merge(savana).merge(reflo).merge(agua)
  ...
  var training_sav = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 3000, 'region': savana.filterBounds(limite), 'scale': 10, 'seed': 1});
...
// print('training_reflo',training_reflo.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDAflora)
                      ...
                      .merge(training_reflo).merge(training_agu)
