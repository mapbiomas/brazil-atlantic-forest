...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'swir1_median','red_edge_1_median','brba_median','ri_median_wet','red_median_wet','green_median','ui_median','msi_median_wet','bsi_median_2','afvi_median','cvi_median','red_edge_1_median_wet','osavi_median_wet','gcvi_median','iia_median_dry','blue_median_dry','osavi_median_dry','blue_median_wet','mbi_median',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(aflora).merge(herb).merge(flor).merge(varzea).merge(agua).merge(agric).merge(reflo).merge(aflora_LANDSAT)
  ...
  var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1100, 'region': herb.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  // Sample the mosaic for the 'aflora' class, using the merged training data image
  ...
  // Sample the mosaic for the 'varzea' class, using the merged training data image
  ...
  // var training_herb = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 200, 'region': n_flo.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  // Sample the mosaic for the reforestation class, using the merged training data image
  ...
  var training_afloraLandsat = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 150, 'region': aflora_LANDSAT.filterBounds(limite), 'scale': 10, 'seed': 1});
...
print('training_agr',training_agr.size())
print('training_herb',training_herb.size())
}
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo).merge(BDherb)//.merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      //complementares
                      .merge(training_afl).merge(training_var).merge(training_herb).merge(training_flo)
                      .merge(training_agr).merge(training_ref)//.merge(training_agu)//
                      .merge(training_afloraLandsat)
