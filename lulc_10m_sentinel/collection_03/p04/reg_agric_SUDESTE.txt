/**
 * This script performs land use classification for the Southeast agricultural region using Random Forest.
 * It leverages stable sample points and embedding bands to build training datasets for target classes.
...
for (var i_ano=0;i_ano<anos.length; i_ano++){
  ...
  var bandNames = ee.List([
                  ...
                  'green_median_wet','spri_median_1','awei_median_dry','awei_median_wet','green_median','evi_median_dry','osavi_median','cvi_median_wet','wetness_median','red_median_wet','ri_median','red_edge_1_median_dry','mbi_median','ndwi_median','blue_median_wet','rvi_median_wet','iia_median_wet','lai_median','osavi_median_wet',
  ...
  // Filter, shuffle, and limit the points to only include forest
  ...
  // Filter the points to only include other non-forest formation
  var BDOutroNFlo = BDamostras.filterMetadata("reference", "equals", 13);
  // Filter, shuffle, and limit the agricultural points
  var BDagro = BDamostras.filterMetadata("reference", "equals", 21);
  BDagro = shuffle(BDagro, 2).limit(2500);
  // Filter the points to only include non-vegetated areas
  ...
  // Filter, shuffle, and limit the water sample points
  var BDagua = BDamostras.filterMetadata("reference", "equals", 33);
  BDagua = shuffle(BDagua, 2).limit(1000);
  // Filter the points to only include herbaceous sandbank
  ...
  var amostraTotal = agric
  // var amostraTotal = campo.merge(mosaico);
  // Convert the primary training vectors to a raster image
  ...
  var training_agr = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': num_agric, 'region': amostraTotal.filterBounds(limite), 'scale': 10, 'seed': 1});
  // var training_campo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 400, 'region': campo.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  var training = BDflo.merge(BDcampo)//.merge(BDreflo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
    .merge(BDagro).merge(BDAflora).merge(BDagua).merge(BDNaoVeg)
    // Complementary samples
    .merge(training_agr)//.merge(training_campo)//.merge(training_mos)//.merge(training_sav).merge(training_flo)
