...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'brightness_median_dry','dswi5_median_wet','ndwi_median_wet','ui_median_wet','spri_median_wet','shape_median_dry','iia_median','nddi_median_dry','gli_median_wet','red_edge_1_median_wet','afvi_median_wet','mbi_median_dry','ndvi_median','ndwi_median_dry','dswi5_median','nddi_median','bsi_median_1','swir1_stdDev','co2flux_median',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = sav.merge(flo).merge(agro).merge(aflora).merge(agua).merge(urban).merge(varzea).merge (campo)
  ...
  // Sample the mosaic for the urban class, using the merged training data image
...
print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      //complementares
                       .merge(training_var).merge (training_cam)
