...
var dirout = 'projects/mapbiomas-brazil/assets/LAND-COVER-10M/COLLECTION-3/GENERAL/classification-mat/';
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'gvmi_median_1','spri_median_wet','evi_median_wet','red_edge_1_median','brba_median_dry','swir2_median_wet','red_edge_1_median_wet','awei_median','wetness_median_wet','brba_median_wet','red_edge_4_median','gvmi_median_dry_1','nddi_median_wet','avi_median','gvmi_median','ndvi_median','swir2_stdDev','ndwi_median_dry','red_edge_3_median_dry',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  // Merge complementary samples
  var amostraTotal = flo.merge(sav).merge(agr).merge(sav2).merge(varzea).merge(flo).merge(agua).merge(aflora_LANDSAT).merge(campo)
  // Reduce complementary samples to an image
  ...
  // Sample the mosaic for the forest class, using the merged training data image
  ...
  // Sample the mosaic for the savanna class, using the merged training data image
  ...
  // Sample the mosaic for the afforestation class, using the merged training data image
  var training_afl = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1000, 'region': off_aflora.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the floodplain forest class, using the merged training data image
  ...
  // Sample the mosaic for the water class, using the merged training data image
  ...
  // Sample the mosaic for the grassland class, using the merged training data image
  var training_campo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 800, 'region': campo.filterBounds(limite), 'scale': 10, 'seed': 1});
  ...
  // Sample the mosaic for the agriculture class, using the merged training data image
  var training_agr = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 3500, 'region': agr.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the afforestation class (Landsat), using the merged training data image
  var training_afloraLANDSAT = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1000, 'region': aflora_LANDSAT.filterBounds(limite), 'scale': 10, 'seed': 1});

// If collecting, print sample sizes
if (coleta) {
...
// print('training_campo',training_campo.size())
// print('training_sav',training_sav.size())
// print('training_afl',training_afl.size())
//print('training_cam',training_campo.size())
...
// print('training_agu',training_agu.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      // .merge(training_afl)
                      .merge(training_var)
                      .merge(training_sav).merge(training_agr).merge(training_flo)
                      .merge(training_agu).merge(training_afloraLANDSAT).merge(training_campo)
