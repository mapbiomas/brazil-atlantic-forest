...
// Define the region ID (reg_28)
var regiaoID = 'reg_28'; // old 11
...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'afvi_median','msi_median','red_median','wetness_median_dry','msi_median_wet','mbi_median_dry','red_edge_2_median','swir2_median_wet','evi_median_dry','ri_median_wet','gvmi_median_dry_1','gcvi_median_dry','spri_median_wet','spri_median','bsi_median_1','ndwi_median_wet','swir1_median_wet','brightness_median_dry','rvi_median_wet',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = varzea.merge(campo).merge(agr).merge(savana).merge(flo).merge(reflo)
  ...
  var training_ref = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 500, 'region': reflo.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the non-forest class
  ...
  var training_agr = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 800, 'region': agr.filterBounds(limite), 'scale': 10, 'seed': 1});
...
// print('training_ref',training_ref.size())
...
// print('training_agr',training_agr.size())
  ...
  var training = BDflo.merge(BDreflo).merge(BDcampo)//.merge(BDsav).merge(BDOutroNFlo).merge(BDAflora)
                      ...
                      .merge(training_cam)
                      // .merge(training_agr)
                      .merge(training_flo)//.merge(training_agu)
                      .merge(training_agr)
