...
var mosaicoTotal = mos.getMosaic(ano,limite);
  ...
  var bandNames = ee.List([
                  ...
                  'brightness_median_dry','avi_median_wet','avi_median_dry','wetness_median_dry','dswi5_median_wet','gvmi_median_dry_1','nddi_median_wet','dswi5_median','red_edge_1_median_dry','red_edge_4_stdDev','nddi_median_dry','afvi_median_dry','swir2_median_dry','gli_median','ratio_median','nddi_median','red_edge_4_median','swir2_median_wet','gemi_median_wet',
...
Map.addLayer(mosaicoTotal.clip(limite.geometry()), visParMedian2, 'Img_Year_'+ano, false);
  ...
  var amostraTotal = agro.merge(flo).merge(sav).merge(varzea).merge(aflora_LANDSAT).merge(campo2).merge(agua).merge(naoVeg)
  
  // Convert the samples to an image
  var amostraTotalimg = amostraTotal.reduceToImage({properties: ['reference'],reducer: ee.Reducer.first()})
  // Keep only the reference band from the reduced image
  amostraTotalimg = amostraTotalimg.select([0],['reference'])
  // Sample the mosaic for each class
  ...
  var training_sav = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 500, 'region': sav.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the floodplain forest class
  var training_var = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 700, 'region': varzea.filterBounds(limite), 'scale': 10, 'seed': 1});
  // var training_reflo = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1000, 'region': off_reflo.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the grassland class
  var training_cam = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 700, 'region': campo2.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the agriculture class
  var training_agr = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 700, 'region': agro.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the afforestation class (Landsat)
  var training_aflLandsat = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 1000, 'region': aflora_LANDSAT.filterBounds(limite), 'scale': 10, 'seed': 1});
  // Sample the mosaic for the non-vegetated class
  var training_naoVeg = mosaicoTotal.select(bandNames).addBands(amostraTotalimg).sample({'numPixels': 400, 'region': naoVeg.filterBounds(limite), 'scale': 10, 'seed': 1});
  
  // If collecting data, print the size of the training samples
  if (coleta) {
  // print('BDflo_'+ano,BDflo.size())
  ...
  //print('training_flo_'+ano,training_flo.size())
  ...
  var training = BDflo.merge(BDsav).merge(BDreflo).merge(BDcampo)//.merge(BDOutroNFlo).merge(BDAflora)
                      .merge(BDagro).merge(BDNaoVeg).merge(BDagua)
                      //complementares
                      .merge(training_cam)
                      .merge(training_agr).merge(training_var)
                      .merge(training_sav).merge(training_flo)
                      .merge(training_naoVeg)
                      
                      .merge(training_aflLandsat)
  ...
  if (coleta) {  
    ...
    // Load the MapBiomas collection 10 integration image
    ...
    // Add the specific year layer from the collection 10 integration image
    Map.addLayer(col10.select('classification_2023'), vis, 'col10', false)
  ...
  // Create an empty blank image for masking
  var blank = ee.Image(0).mask(0);
