// Summary: This script extracts satellite embedding data from Google Earth Engine (GEE) using stable sample points across different regions
// within the Atlantic Forest biome. It loops through a specified sequence of years (2017-2025) and region codes, filters an annual satellite 
// embedding collection by bounding geometry and dates, combines them with region-specific training point features, and exports the sampled 
// properties as GEE asset tables for subsequent classification tasks.
...
for (var i_ano=0;i_ano<anos.length; i_ano++){
    ...
    var ano = anos[i_ano];
        ...
        //mosaico APENAS embeddings
        // Load the annual satellite embedding image collection
        var mosEMB = ee.ImageCollection("GOOGLE/SATELLITE_EMBEDDING/V1/ANNUAL")
        
      
        // Filter the collection by the Atlantic Forest boundary spatial constraint
        mosEMB = mosEMB.filterBounds(limite_MA)
        // Filter the collection by the start and end dates of the current year
        mosEMB = mosEMB.filterDate(ano+'-01-01', ano+'-12-31')
        // Mosaic the filtered image collection into a single image
        mosEMB = mosEMB.mosaic()
        
        // Create a constant image representing the current year
        var img_ano = ee.Image(ano); // Add year band.
        // Append the year band to the satellite embedding mosaic image
...
var regioes_lista = [
      ['reg_01']
      ,['reg_02'],['reg_03'],['reg_04'],['reg_05'],['reg_06'],['reg_07'],['reg_08'],['reg_09'],['reg_10'],['reg_11'],['reg_12'],['reg_13'],['reg_14'],['reg_15'],
